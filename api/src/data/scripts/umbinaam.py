...
def calculate_umbinaam(player_stats, analytics):
    ...
    net_rating_z_score = calculate_z_score(net_rating, Decimal(analytics[0]), analytics[1])
    ...
    ws_per_48_z_score = calculate_z_score(ws_per_48, Decimal(analytics[6]), analytics[7])
    net_rating_z_score = calculate_z_score(net_rating, Decimal(analytics[8]), analytics[9])
    bpm_z_score = calculate_z_score(bpm, Decimal(analytics[10]), analytics[11])
    vorp_z_score = calculate_z_score(vorp, Decimal(analytics[12]), analytics[13])
    epm_z_score = calculate_z_score(epm, Decimal(analytics[14]), analytics[15])
    e_wins_z_score = calculate_z_score(e_wins, Decimal(analytics[16]), analytics[17])
    lebron_z_score = calculate_z_score(lebron, Decimal(analytics[18]), analytics[19])
    bbi_war_z_score = calculate_z_score(bbi_war, Decimal(analytics[20]), analytics[21])
    rpm_z_score = calculate_z_score(rpm, Decimal(analytics[22]), analytics[23])
    rpm_wins_z_score = calculate_z_score(rpm_wins, Decimal(analytics[24]), analytics[25])
    wpa_z_score = calculate_z_score(wpa, Decimal(analytics[26]), analytics[27])
    raptor_z_score = calculate_z_score(raptor, Decimal(analytics[28]), analytics[29])
    raptor_war_z_score = calculate_z_score(raptor_war, Decimal(analytics[30]), analytics[31])
    ...
    umbinaam = ((net_rating_z_score * Decimal(0.25)) + pie_z_score + (per_z_score * Decimal(0.25)) + ws_per_48_z_score + bpm_z_score + (vorp_z_score * Decimal(0.75))
                + epm_z_score + (e_wins_z_score * Decimal(0.50)) + lebron_z_score + (bbi_war_z_score * Decimal(0.50)) + rpm_z_score + (rpm_wins_z_score * Decimal(0.50))
                + wpa_z_score + raptor_z_score + (raptor_war_z_score * Decimal(0.50)) + fic_z_score + (dpm_z_score * Decimal(0.75))) / Decimal(13.00)
    return umbinaam
